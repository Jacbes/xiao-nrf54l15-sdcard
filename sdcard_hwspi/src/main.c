...
int main(void)
{
	...
	printk("\n");
	printk("========================================================\n");
	printk(" XIAO nRF54L15  --  SD card over HARDWARE SPI (SPIM22)\n");
	printk(" Pins: SCK=P1.05(D1) MOSI=P1.06(D2) MISO=P1.07(D3) CS=P1.04(D0)\n");
	printk(" Console stays on UART20 -> USB / COM (no conflict)\n");
	printk("========================================================\n");
	...
	while (!card_responds()) {
		if (waits == 0) {
			...
			printk("Check connections: SCK=P1.05/D1, MOSI=P1.06/D2, "
			       "MISO=P1.07/D3, CS=P1.04/D0, plus 3V3 and GND.\n");
